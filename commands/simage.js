...
try {
    ...
    console.log('Sharp module not available, image processing features will be limited');
    sharp = null;
}
const fs = require('fs');
...
const convertStickerToImage = async (sock, quotedMessage, chatId) => {
    try {
        if (!sharp) {
            await sock.sendMessage(chatId, { text: 'Sorry, image conversion is not available due to missing dependencies.' });
            return;
        }
