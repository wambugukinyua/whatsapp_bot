...
async function songCommand(sock, chatId, message) {
   try {
      ...
      // Download using savetube with retry logic (up to 3 attempts)
      let result;
      let downloadLinkAttempts = 0;
      const maxRetries = 3;

      while (downloadLinkAttempts < maxRetries) {
         try {
            downloadLinkAttempts++;
            ...
            if (downloadLinkAttempts >= maxRetries) {
               return await sock.sendMessage(chatId, { text: `Failed to fetch download link after ${maxRetries} attempts. Try again later.` });
            }
            // Wait a bit before retrying (exponential backoff)
            await new Promise(resolve => setTimeout(resolve, downloadLinkAttempts * 1000));
         }
      }
      ...
      // Download the MP3 file with retry logic (up to 3 attempts)
      const tempDir = path.join(__dirname, '../temp');
      if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir);
      const tempFile = path.join(tempDir, `${Date.now()}.mp3`);

      let fileDownloadAttempts = 0;
      let downloadSuccess = false;

      while (fileDownloadAttempts < maxRetries && !downloadSuccess) {
         try {
            ...
            if (fileDownloadAttempts >= maxRetries) {
               return await sock.sendMessage(chatId, { text: `Failed to download the song file after ${maxRetries} attempts. Please try again later.` });
            }

            // Wait before retrying (exponential backoff)
            await new Promise(resolve => setTimeout(resolve, fileDownloadAttempts * 1500));
         }
      }
