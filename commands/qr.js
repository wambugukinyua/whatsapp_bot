...
*Commands:*
• \`.qr generate <text>\` - Generate QR code from text
• \`.qr scan\` - Reply to an image to scan QR code

*Examples:*
• \`.qr generate Hello World\`
• \`.qr generate https://github.com\`
• \`.qr scan\` (reply to image containing QR code)

*Features:*
✅ Generate QR codes from any text
✅ Scan QR codes from images
✅ Support for URLs, text, and more
✅ High-quality PNG output`;
        ...
        // QR Code Scanner
            ...
            if (!imageMessage) {
                return await sock.sendMessage(chatId, { 
                    text: "❌ The replied message doesn't contain an image!" 
                });
            }
            ...
            try {
                ...
                if (code) {
                    ...
                    let responseText = `*QR Code Scanned Successfully!* ✅\n\n`;
                    responseText += `📝 *Content:* ${qrData}\n\n`;
                    
                    // Detect content type
                    if (qrData.startsWith('http://') || qrData.startsWith('https://')) {
                        responseText += `🔗 *Type:* URL/Website Link\n`;
                        responseText += `🌐 *Preview:* ${qrData}\n\n`;
                    } else if (qrData.includes('@') && qrData.includes('.')) {
                        responseText += `📧 *Type:* Email Address\n\n`;
                    } else if (qrData.startsWith('tel:')) {
                        responseText += `📞 *Type:* Phone Number\n\n`;
                    } else if (qrData.startsWith('wifi:')) {
                        responseText += `📶 *Type:* WiFi Configuration\n\n`;
                    } else if (qrData.startsWith('geo:')) {
                        responseText += `📍 *Type:* Geographic Location\n\n`;
                    } else {
                        responseText += `📄 *Type:* Plain Text\n\n`;
                    }
                ...
                await sock.sendMessage(chatId, { 
                    text: "❌ Failed to scan QR code. Please try with a clearer image." 
                });
            }
        }
